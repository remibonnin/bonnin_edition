...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="text()[parent:: tei:ab]" mode="addAnchors">
    ...
    <xsl:template match=" tei:ab | tei:ab" mode="tokenize">
        <xsl:copy>
            <xsl:apply-templates select="attribute::*"/>
            <xsl:attribute name="n"><xsl:number count=" tei:ab  | tei:ab" from="tei:text" level="any"/></xsl:attribute>
    ...
    <xsl:template match="tei:anchor[@type = 'wordBoundary']" mode="tokenize">
        <xsl:choose>
            ...
            <xsl:when
                test="not(preceding-sibling::tei:anchor[@type = 'wordBoundary']) and parent:: tei:ab">
                <w>
                    <xsl:apply-templates 
        ...
        <xsl:if test="not(following-sibling::tei:anchor[@type = 'wordBoundary']) and parent:: tei:ab">
            <w>
                <xsl:apply-templates 
